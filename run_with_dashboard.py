    ...
    async def _start_kalshi_monitoring(self) -> None:
        ...
        async with self.kalshi_client:
            ...
            if self.data_feed and self._kalshi_markets:
                ...
                # Keep this task inside the Kalshi client context. Once matching
                # completes it becomes the paper-scanning loop; it never submits
                # signals to the execution engine.
                await self._run_matching_background(polymarket_markets)

                logger.info(
                    "Starting cross-platform PAPER scanner (auto-execution disabled)"
                )
                while self._running:
                    await self._scan_cross_platform_once()
                    await asyncio.sleep(5)

    async def _scan_cross_platform_once(self) -> None:
        """Evaluate matched pairs from fresh depth and emit alert-ready paper records."""
        if not self.cross_platform_engine or not self.kalshi_client:
            return

        semaphore = asyncio.Semaphore(10)

        async def evaluate(pair):
            async with semaphore:
                try:
                    ...
                    if polymarket_ob is None:
                        polymarket_ob = await self.client.get_orderbook(
                            pair.polymarket_id
                        )
                    kalshi_ob = await self.kalshi_client.get_orderbook_unified(
                        pair.kalshi_ticker
                    )
                    if polymarket_ob is None or kalshi_ob is None:
                        return None
                    return self.cross_platform_engine.check_arbitrage(
                        pair, polymarket_ob, kalshi_ob
                    )
                except Exception as exc:
                    logger.debug("Paper scan failed for %s: %s", pair.pair_id, exc)
                    return None

        results = await asyncio.gather(
            *(evaluate(pair) for pair in self._matched_pairs),
            return_exceptions=False,
        )
        alerts = [
            opportunity.to_alert_dict()
            for opportunity in self.cross_platform_engine.rank_opportunities(
                [result for result in results if result is not None]
            )
        ]
        for alert in alerts:
            # Dedupe identical executable snapshots so a five-second scan does
            # not create an alert storm.
            economics = alert["economics"]
            fingerprint = (
                alert["pair"]["id"],
                economics["contracts"],
                economics["total_cost"],
                economics["conservative_net_profit"],
            )
            seen = getattr(self, "_cross_alert_fingerprints", set())
            if fingerprint in seen:
                continue
            seen.add(fingerprint)
            self._cross_alert_fingerprints = set(list(seen)[-500:])
            dashboard_state.add_cross_platform_opportunity(alert)
